...
uint64_t ex_jal_inst_nr;  // 由于流水线原因，ebreak指令后面会多一条jal指令，导致ex_jal_inst_nr比jal_inst_nr多1，所以初始化为-1
uint64_t ex_taken_br_nr;

uint64_t ex_taken_final_br_nr, ex_taken_final_jalr_nr;
uint64_t ex_taken_final_jal_nr;  // 由于流水线原因，ebreak指令后面会多一条jal指令，导致ex_taken_final_jal_nr比ex_jal_inst_nr多1，所以初始化为-1
