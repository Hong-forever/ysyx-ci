...
static void out_of_bound(paddr_t addr, bool is_write) {
    printf(COLOR_RED "%s address = 0x%08x is out of bound of the size 0x%08x from pmem [0x%08x, 0x%08x]\n" COLOR_END, is_write?"Write":"Read", addr, CONFIG_MSIZE, PMEM_LEFT, PMEM_RIGHT);
    assert(0);
}
