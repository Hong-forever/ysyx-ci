...
void __am_timer_init()
{
    ...
    us = (((uint64_t)time_hi << 32) | (uint64_t)time_lo) / 500ULL;
}
...
void __am_timer_uptime(AM_TIMER_UPTIME_T *uptime)
{
    ...
    uptime->us = (((uint64_t)time_hi << 32) | (uint64_t)time_lo) / 500ULL;
}
...
void __am_timer_rtc(AM_TIMER_RTC_T *rtc)
{
    uint32_t time_hi = inl(RTC_PORT+ 4);
    uint32_t time_lo = inl(RTC_PORT);
    uint64_t up_us = (((uint64_t)time_hi << 32) | (uint64_t)time_lo) / 500ULL;
