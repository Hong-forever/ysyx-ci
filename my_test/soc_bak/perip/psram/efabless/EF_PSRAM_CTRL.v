...
module PSRAM_READER (
    ...
    wire[1:0] byte_index = qpi_mode ? {counter[7:1] - 8'd7}[1:0] : {counter[7:1] - 8'd10}[1:0];
    always @ (posedge clk)
        if(qpi_mode ? (counter >= 8'd14 && counter <= FINAL_COUNT) : (counter >= 8'd20 && counter <= FINAL_COUNT))
            if(sck)
...
module PSRAM_WRITER (
    ...
    assign dout     =   qpi_mode ?
                        ...
                        (counter == 14) ?   line[31:28]         : line[27:24]
                        :
                        ...
                        (counter == 20) ?   line[31:28]         : line[27:24];
...
module PSRAM_QPI_MODE (
    ...
    always @ (posedge clk or negedge rst_n)
        if(!rst_n) begin
            switch_done    <= 1'b0;
        end else if(counter == FINAL_COUNT + 1) begin
            switch_done    <= 1'b1;
        end
