...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , CSR, R(rd) = CSR(csr_idx); CSR(csr_idx) = src1; printf("csrrw: idx=0x%03x, val=0x%08x\n", csr_idx, src1));
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , CSR, R(rd) = CSR(csr_idx); if(rs1 != 0) CSR(csr_idx) |= src1; printf("csrrs: idx=0x%03x, val=0x%08x\n", csr_idx, CSR(csr_idx))); 
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , CSR, R(rd) = CSR(csr_idx); if(rs1 != 0) CSR(csr_idx) &= ~src1; printf("csrrc: idx=0x%03x, val=0x%08x\n", csr_idx, CSR(csr_idx)));
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , CSR, R(rd) = CSR(csr_idx); CSR(csr_idx) = imm; printf("csrrwi: idx=0x%03x, val=0x%08x\n", csr_idx, imm));
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , CSR, R(rd) = CSR(csr_idx); if(imm != 0) CSR(csr_idx) |= imm; printf("csrrsi: idx=0x%03x, val=0x%08x\n", csr_idx, CSR(csr_idx)));
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , CSR, R(rd) = CSR(csr_idx); if(imm != 0) CSR(csr_idx) &= ~imm; printf("csrrci: idx=0x%03x, val=0x%08x\n", csr_idx, CSR(csr_idx)));
