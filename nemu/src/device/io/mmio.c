...
void add_mmio_map(const char *name, paddr_t addr, void *space, uint32_t len, io_callback_t callback) {
  ...
  if (in_rom(left) || in_rom(right)) {
    report_mmio_overlap(name, left, right, "rom", PMEM_LEFT_ROM, PMEM_RIGHT_ROM);
  }
  if (in_ram(left) || in_ram(right)) {
    report_mmio_overlap(name, left, right, "ram", PMEM_LEFT_RAM, PMEM_RIGHT_RAM);
  }
